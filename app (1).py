...
class PlotPlanExtractor:
    ...
    def _parse_text(self, text):
        ...
        # Sidewalk
        sidewalk_patterns = [
            r"(\d+(?:\.\d+)?)\s*[xX×]\s*(\d+(?:\.\d+)?)\s*(?:SIDEWALK|WALK)",
            r"(?:SIDEWALK|WALK)[^\d]*(\d+(?:,\d+)?(?:\.\d+)?)\s*(?:SF|S\.F\.)",
        ]
        for pattern in sidewalk_patterns:
            matches = re.findall(pattern, text_upper)
            if matches:
                if isinstance(matches[0], tuple):
                    self.measurements['sidewalk_sqft'] = float(matches[0][0]) * float(matches[0][1])
                else:
                    self.measurements['sidewalk_sqft'] = float(matches[0].replace(',', ''))
                break

        # Apron
        apron_patterns = [
            r"(?:APRON)[^\d]*(\d+(?:\.\d+)?)\s*[xX×]\s*(\d+(?:\.\d+)?)",
            r"(?:APRON)[^\d]*(\d+(?:,\d+)?(?:\.\d+)?)\s*(?:SF|S\.F\.)",
        ]
        for pattern in apron_patterns:
            ...
            if matches:
                if isinstance(matches[0], tuple):
                    ...
                    self.measurements['apron_sqft'] = float(matches[0].replace(',', ''))
                break

        # Curb
        curb_pattern = r"(?:CURB|D-CURB)[^\d]*(\d+(?:\.\d+)?)\s*(?:LF|L\.F\.)?['"]?"
        ...
        # Driveway
        driveway_patterns = [
            r"(?:DRIVEWAY)[^\d]*(\d+(?:\.\d+)?)\s*[xX×]\s*(\d+(?:\.\d+)?)",
            r"(?:DRIVEWAY)[^\d]*(\d+(?:,\d+)?(?:\.\d+)?)\s*(?:SF|S\.F\.)",
        ]
        for pattern in driveway_patterns:
            matches = re.findall(pattern, text_upper)
            if matches:
                if isinstance(matches[0], tuple):
                    self.measurements['driveway_sqft'] = float(matches[0][0]) * float(matches[0][1])
                else:
                    self.measurements['driveway_sqft'] = float(matches[0].replace(',', ''))
                break
